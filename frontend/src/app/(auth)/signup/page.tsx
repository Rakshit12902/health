    ...
    <div className="min-h-screen bg-[var(--color-bg-primary)] flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <p className="text-[var(--color-text-muted)]">Join CuraMind today</p>
        </div>

        <div className="glass-panel p-8">
          {error && <div className={`mb-4 p-3 rounded-lg text-sm border ${error.includes('check your email') ? 'bg-[var(--color-success)]/20 border-[var(--color-success)]/50 text-[var(--color-success)]' : 'bg-[var(--color-danger)]/20 border-[var(--color-danger)]/50 text-[var(--color-danger)]'}`}>{error}</div>}
          ...
          <p className="mt-6 text-center text-sm text-[var(--color-text-muted)]">
            Already have an account? <Link href="/login" className="text-[var(--color-accent-cyan)] hover:underline">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
